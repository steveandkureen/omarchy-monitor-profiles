...
import qs.Ui
import qs.Commons
// Display's own helpers (brightness/scale math), unmodified from the clone,
// just renamed so they don't collide with this plugin's own Model.js below.
import "DisplayModel.js" as Model
import "Model.js" as ProfileModel

// Clone of Omarchy's first-party Display widget (`omarchy plugin clone
// omarchy.monitor`), with the bottom "Displays" list (enable/disable a
// connected monitor) replaced by this plugin's own saved-profile list.
// Brightness/text-size/scale sections below are unmodified Display source.
//
// moduleName/ipcTarget are deliberately left as "omarchy.monitor" (not this
// plugin's own id) -- that's how a clone actually takes over the original's
// bar slot and SUPER+CTRL+D keybind: the shell's IPC layer routes calls
// targeting "omarchy.monitor" to whichever plugin currently declares
// omarchy.clonedFrom = "omarchy.monitor" in its manifest (see manifest.json).
// Reusing this plugin's own id here instead would just be a second, unrelated
// IpcHandler -- it wouldn't make this the thing SUPER+CTRL+D or the Display
// bar slot actually open.
Panel {
  id: root
  ...
  // ---- Monitor Profiles: saved layouts, replacing Display's own
  // ---- enable/disable-a-display list at the bottom of the panel.
  ...
  function refreshProfiles() {
    ...
    profileActiveProc.running = false
    profileActiveProc.running = true
  }

  // Same validate-then-build-path pattern as EditorView.qml's saveProfile/
  // deleteProfile and Panel.qml's applyProfileByName -- profileNames always
  ...
  function applyProfile(name) {
    if (!ProfileModel.isValidProfileName(name)) return
    root.applyTarget = name
    profileApplyFile.path = root.profilesDir + "/" + name + ".conf"
  ...
  //   "profiles"   - vertical list of saved Monitor Profiles layouts;
  //                  j/k walks each row, Enter/click applies one. Replaces
  //                  Display's own "monitors" (enable/disable a display)
  //                  section -- same slot in the cursor model, same list
  //                  shape, different data and action.
  ...
  function sectionCount(section) {
    ...
    // (scaleValues || []) / (profileNames || []): onProfileNamesChanged
    // below can fire at this object's own construction, when profileNames
    // first settles from its declared [] default -- QML doesn't guarantee
  ...
  // ---- Monitor Profiles plumbing (list/apply saved layouts) --------------
  ...
  Process {
    id: profileActiveProc
    command: ["bash", "-lc", "cat \"" + root.hyprScreenLuaPath + "\" 2>/dev/null"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        root.activeProfileName = ProfileModel.activeProfileNameFromLua(String(text || ""))
      }
    }
  }
  ...
  FileView {
    ...
    printErrors: false
    onLoaded: {
      var monitors = ProfileModel.parseProfileText(text())
      if (monitors.length === 0) return
      profileApplyLuaFile.path = root.hyprScreenLuaPath
      profileApplyLuaFile.setText(ProfileModel.profileToLua(monitors, root.applyTarget))
      profileReloadProc.running = true
    }
  }

  FileView {
    id: profileApplyLuaFile
    watchChanges: false
    printErrors: false
  }

  Process {
    id: profileReloadProc
